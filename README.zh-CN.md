...
完成框选后，点击工具栏的矩形图标或按 `R` 进入矩形标注，再在选区内
拖拽画框；按住 `Shift` 可画正方形。支持在跨屏选区内连续绘制。

- 在第二行直接点击色块选择红、橙、黄、绿、蓝、黑、白；点击圆点选择 1、3、5 个逻辑像素的线宽。
- `Ctrl+Z` 撤销，`Ctrl+Y` 或 `Ctrl+Shift+Z` 重做，不占用工具栏按钮。
- 绘制中按 `Esc` 取消当前矩形；再次按 `Esc` 或按 `R` 退出矩形工具，已完成的标注保留。
- 退出矩形工具后重新框选会清除旧选区的标注。
- 复制和保存包含标注；OCR 读取未标注的原图。

目前提供矩形描边，暂不支持移动、缩放、旋转、填充和圆角等二次编辑。
